...
## USAGE SYNTAX
# Generate reports for ALL inspection folders found in inspections/
python generate_report.py
...
## Image Canvas Rules: The 10x2 Photo Canvas Grid
VIN Isolation Routine: When searching your target unit's photos/ folder, the script checks for any filename containing "vin" or "pin". If identified, that image is intercepted and locked directly into Slot 1 (Top Left) on the first photo page with a VIN / PIN IMAGE identifier overlay text box.

Aspect Ratio Uniformity: Photo elements are scaled using an optimization thumbnail loop (ImageOps.exif_transpose and Image.LANCZOS). Images fit within their designated bounding areas without experiencing stretching or distortion distortions.
